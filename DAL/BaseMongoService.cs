...
namespace DAL
{
    public abstract class BaseMongoService<T> : IBaseService<T>
    {
        ...
        //public Task<IEnumerable<T>> FindAll(params string[] excludedFields)
        //{
        //    throw new NotImplementedException();
        //}
        ...
        public async Task<T> FindActiveById(string id)
        {
            try
            {
                ...
                var filter = builder.Eq("_id", new ObjectId(id)) & builder.Eq("DeleteFlag", false);
        ...
        {            
            ...
            {
                var filter = Builders<T>.Filter.Eq("_id", new ObjectId(id));
                var update = Builders<T>.Update.Set("DeleteFlag", true);
